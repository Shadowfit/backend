...
| **Projection (JSON off-page)** | 세션 분석 계산이 안 쓰는 JSON 컬럼(2.3KB)까지 로드하고 있었음. off-page(오버플로우 페이지) 랜덤 I/O를 projection으로 회피(측정 시점 3컬럼, 현재 4컬럼). AWS(m6i.xlarge)에서 실제 1억 행 × 실제 2.3KB JSON으로 재검증 | payload **1,740.1KB → 22.6KB (−98.7%)**, warm 쿼리 **40.6ms → 1.4ms**(최대 41배). 세션 종료 시 1회 도는 비동기 precompute라, 개선 의미는 체감 지연 감소가 아니라 배치 잡 자원 소모 감소<br>⚠️ **정상 리포트 조회는 이 쿼리를 안 탄다**(precompute·폴백 전용). **−98.7% 는 DB→앱 payload**(warm, 750행/세션, 3컬럼 시절) — [조건 전체](../docs/portfolio/realmysql-experiments.md#projection-98-7) |
