 ...
 * 두 요청이 같은 세션을 건드렸다는 게 로그에서 바로 읽힌다. 다만 이건 경쟁의 <b>단서지 증거가 아니다</b> —
 * 순차적으로 건드려도 같은 모양이 나온다. 실제 충돌 여부는 {@code SessionMetrics} 의 낙관락 충돌
 * 지표가 판정하고, cid는 그 충돌이 잡혔을 때 상대가 누구였는지 되짚는 용도다.
 *
