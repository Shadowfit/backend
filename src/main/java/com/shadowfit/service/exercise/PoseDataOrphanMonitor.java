 ...
 * DELETE 의 파편화가 (이 주석을 쓸 당시) 미검증이었다. 그래서 <b>고치기 전에 세는 것</b>이 먼저다.
 * 이 클래스가 sweep 으로 승격되면 같은 쿼리에 {@code DELETE} 만 붙이면 된다.
 *
 * <p><b>파편화 쪽 막힘은 좁혀졌다 — 다만 풀리지는 않았다 (2026-08-09 실측).</b>
 * {@code loadtest/results/delete-fragmentation-2026-08-09/}. 소량 반복 DELETE 는 파편화를 무한
 * 누적시키지 않는다 — FIFO 삭제는 아예 누적이 없고, sweep 이 만들 모양인 <b>구멍 뚫기</b> 삭제도
 * <b>+24% 계단이 한 번</b> 생긴 뒤 평탄해진다(구멍 밀도를 20배 올려도 같다). 즉 "sweep 을 돌리면
 * 파편화가 계속 쌓인다" 는 우려는 기각됐다.
 *
 * <p>⚠️ 그래도 <b>승격 근거로는 부족하다.</b> 그 측정은 {@code outbox_events} 행 모양(≈108 B/행)으로
 * 했는데 {@code pose_data} 는 {@code joint_coordinates} JSON 이 들어가 훨씬 커서 페이지당 행 수가
 * 다르고, 이 테이블의 DELETE 는 살아있는 파티션 안에서 일어나 {@code PoseDataPartitionScheduler}
 * 의 DROP 과 상호작용한다. 둘 다 재지 않았다. <b>승격 여부는 여전히 별개 결정이고, 그 판단의
 * 1순위 입력은 아래 게이지가 세는 실제 발생 빈도다.</b>
 *
