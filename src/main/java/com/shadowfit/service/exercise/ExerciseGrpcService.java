...
public class ExerciseGrpcService extends ExerciseServiceGrpc.ExerciseServiceImplBase {
     ...
     *
     * <p>🔶 <b>2026-08-23 시점: 3·4·5 의 순위는 못 낸 상태였다</b> — 팔당 3판이라 상한 4 의
     * 산포(18·37·37)가 3 의 구간(30~36)과 겹쳤다.
     *
     * <p>🔵 <b>2026-08-26: 기본값을 3 → 5 로 올렸다</b>(사용자 confirm). 팔당 판을 3 → 6 으로
     * 늘려 다시 잰 결과({@code loadtest/results/r276-ceiling-rank-aws-2026-08-26/} — 상한
     * 3·4·5 × 7블록, 첫 블록 버림, 라틴 방격, 동시성 16 고정)가 근거다:
     *
     * <pre>
     *   상한 3   잔여 8.0%   retried 400.5   ← 옛 값
     *   상한 4   잔여 5.9%   retried 533.0
     *   상한 5   잔여 2.5%   retried 479.5   ← 채택
     * </pre>
     *
     * 중앙값이 이제 <b>3 &gt; 4 &gt; 5 로 단조롭고</b>, 같은 블록 안 짝비교로는 <b>5 가 4 를
     * 6/6, 3 을 5/6 에서 이긴다</b> — 2026-08-23 라운드가 판 부족으로 못 가른 자리다.
     *
     * <p>🔴 <b>그래도 열려 있는 것 셋</b> (전부 이 값을 다시 볼 이유가 될 수 있다):
     * <ul>
     *   <li>같은 조건인데 이 라운드의 절대 잔여율이 2026-08-23 라운드와 어긋난다(3: 6.2%→8.0%,
     *       4: 7.4%→5.9%, 5: 4.2%→2.5% — 방향이 다 같지 않다). 원인 미상 — 인스턴스 개체차·
     *       그 사이 main 변경·순수 산포 중 하나</li>
     *   <li>두 겹(이 상한 + AI 재전송 3회)이 다 소진된 뒤의 실사용 영향은 여전히 안 쟀다
     *       (#276 ③ 잔여 — {@code slo-baseline.md} 미확정)</li>
     *   <li>5 는 3 보다 DB 시도를 약 20% 더 쓴다(retried 400.5→479.5) — 그 자체가 다른
     *       트래픽에 주는 부담은 안 쟀다</li>
     * </ul>
     */
    @Value("${shadowfit.pose.deadlock.max-retries:5}")
    private int deadlockMaxRetries = 5;   // 초기값도 둔다 — Spring 밖에서 만들면 0 이 되고, 그 0 은 «재시도 없음» 이다
